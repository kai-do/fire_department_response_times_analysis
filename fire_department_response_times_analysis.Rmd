...
```{r}
basic_incident_df_2 <- basic_incident_2014_df %>%
  mutate(INC_DATE = case_when(nchar(INC_DATE) == 7 ~ as.POSIXct(strptime(paste0("0", INC_DATE), format = "%m%d%Y")),
                              TRUE ~ as.POSIXct(strptime(INC_DATE, format = "%m%d%Y"))),
         ALARM = case_when(nchar(ALARM) == 11 ~ as.POSIXct(strptime(paste0("0", ALARM), format = "%m%d%Y%H%M")),
                              TRUE ~ as.POSIXct(strptime(ALARM, format = "%m%d%Y%H%M"))),
         ARRIVAL = case_when(nchar(ARRIVAL) == 11 ~ as.POSIXct(strptime(paste0("0", ARRIVAL), format = "%m%d%Y%H%M")),
                              TRUE ~ as.POSIXct(strptime(ARRIVAL, format = "%m%d%Y%H%M"))),
         LU_CLEAR = case_when(nchar(LU_CLEAR) == 11 ~ as.POSIXct(strptime(paste0("0", LU_CLEAR), format = "%m%d%Y%H%M")),
                              TRUE ~ as.POSIXct(strptime(LU_CLEAR, format = "%m%d%Y%H%M"))))

saveRDS(basic_incident_df_2, file = "./datasets/nfirs/rds/basicincident_2014.rds")


incident_type_counts_df <- basic_incident_2017_df %>%
  count(STATE, FDID, INC_TYPE, sort = TRUE)

incident_counts_df <- basic_incident_2017_df %>%
  count(STATE, FDID, sort = TRUE)
  
incident_type_counts_df <- inner_join(fd_df, incident_type_counts_df)
incident_counts_df <- inner_join(fd_df, incident_counts_df)

incident_type_counts_df <- incident_type_counts_df %>%
